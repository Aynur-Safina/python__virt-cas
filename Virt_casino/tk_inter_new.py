...
def r_b():
    ...
    us_text_entry.pack()
    play_button_rb.pack(pady=10)


def play_rb():
    global us_rb_origin, us_rb_remove, us_rb_lower, bet, bet_entry, us_text_entry, lbl_instruction,lbl_win
    ...
    us_rb_origin=us_text_entry.get()
    print(us_rb_origin)
    ...
    if us_rb_lower == rand_list_rb:  # Рассматриваем вариант выигрыша
        lbl_win['text']='Поздравляем!Сыграло {rand_list_rb} поле.\n'
        'Ваш выигрыш {bet * 2} фантиков\nСыграем еще раз?'


    elif us_rb_lower == losing:  # Рассматриваем вариант проигрыша
        lbl_win['text'] =('Увы, сыграло {rand_list_rb} поле.\n'
                          ' Попробуйте еще раз, удача обязательно улыбнется Вам!\n'
                          'Сыграем еще раз?')
...
us_num_entry = Entry(root,
            ...
            relief='solid',
            width=20)
play_button_rb= Button(root, text='Играть',
                         font='Consolas 16',
                         fg='black',
                         bg='green',
                         justify='center',
